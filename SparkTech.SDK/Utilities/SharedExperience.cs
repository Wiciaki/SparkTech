namespace SparkTech.SDK.Utilities
{
    public class SharedExperience
    {
        ...
        public static void OnGameUpdate(EventArgs args)
        {
            ...
            foreach (var hero in ObjectManager.Get<IHero>())
            {
                i += 1;
                ...
                if (InvisibleCount[i] > SharingCount[i] - 1 && SharingCount[i] > 0)
                    InvisibleCount[i] = SharingCount[i] - 1;
            ...
            foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>())
            {
                ...
                if ((Environment.TickCount - TimeChangedExp[i]) < 5000 && InvisibleCount[i] > 0)
                {
                    if (menu.Item("drawPredictionCircle").GetValue<bool>())
                        Render.Circle.DrawCircle(LastMinionPosition[i], 1500, Color.Red);
