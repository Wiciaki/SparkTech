...
namespace SparkTech.Geometry.AOE
{
    using System;
    ...
    public class MEC
    {
        ...
        //TODO
        public static Circle GetAcurateMEC(List<AIBaseClient> enemies)
        {
            var points = new List<Vector2>();

            foreach (var enemy in enemies)
            {
                points.AddRange(GetPointsOnBoundingRadius(enemy, 3));
            }

            var convexHull = MakeConvexHull(points);
            FindMinimalBoundingCircle(convexHull, out var center, out var radius);

            return new Circle(center.ToFlat3D(), radius);
        }

        //TODO
        public static Circle GetMEC(List<Vector3> points)
        {
            var convexHull = MakeConvexHull(points.Select(e => e.To2D()).ToList());
            FindMinimalBoundingCircle(convexHull, out var center, out var radius);
        ...
        /// <summary>
        ///     Find a circle through three Vector points
        /// </summary>
        /// <param name="a">Vector point A</param>
        /// <param name="b">Vector point B</param>
        /// <param name="c">Vector point C</param>
        /// <param name="center">Returned Vector Center</param>
            ...
            for (var i = 0; i < hull.Count - 2; i++)
            {
                for (var j = i + 1; j < hull.Count - 1; j++)
                {
                    for (var k = j + 1; k < hull.Count; k++)
                    {
                        ...
                        FindCircle(hull[i], hull[j], hull[k], out var testCenter, out var testRadius2);
        ...
        private static IEnumerable<Vector2> GetPointsOnBoundingRadius(AIBaseClient target, uint quality)
        {
            var returnList = new List<Vector2>();
            var offsetDegree = 360f / quality;

            for (var i = 0; i < quality; i++)
            {
                returnList.Add(PointOnCircle(target.BoundingRadius, offsetDegree * i, target.Position.To2D()));
            }
